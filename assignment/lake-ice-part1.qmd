...
ggplot(sunapee, aes(x=year, y = ice_off_doy))+
  ...
  geom_line(color = "red")+
  labs(
    title = "Lake Sunapee Ice off days throughout the years",
    x = "Year",
    y =  "Day at Which no Ice was in the Lake")
```
...
**Answer 2:** I think the sign will be + becuase there is going to be ups and downs in the relationship which means you need to add the different parts to get an overall answer.
...
**Answer 3:**.   The slope is going down meaing the ice off days is getting earlier everyday.

# Is our model any good? 
No becuase we have small R2
...
**Answer 4:**.  The R2 is  0.2055, The are not strongky related becuase of the small R2. The R2 needs to be closer to 1 to have a storng relationship.
...
sunapee |>
ggplot()+
  geom_histogram(aes( x= residual), bins = 10)+
  labs(
    title = "Ice Off Day Residuals",
    x = "Residuals",
    y = "Number of the Different Residuals"
...
**Answer 7:** The P value is 2.504e-09
...
**Answer 8:**.  Based on the slope value a 1 week change would be around 66 years due to the -0.106 slope. And the Pvalue confirms that it is meaningful because the pvalue is close to 0.
...
**Answer 9:**  I am feeling good based off my P value being close to 0 and with a negtive slope because with global warming the slope should be negative for the ice off day.
...
ggplot(sunapee, aes(x =year,y=ice_off_doy))+
  ...
  geom_abline(
    ...
    color = "red")+
  labs(
    title = "Ice Off Day Compared To The Year",
    x= "Year",
    y = "Ice Off Day"
  )
...
- How wide is your confidence interval (number of days)?  33.6 days

**Answer 11:**

```{r}
iceoff_2040 <- coef(fit) [1] +coef(fit)[2] * 2040

s_quamtile <- quantile(sunapee$residual,0.05)
L_quamtile <- quantile(sunapee$residual,0.95)

Lower_c <- iceoff_2040 + s_quamtile
Higher_c <- iceoff_2040 + L_quamtile

Wideness <- Higher_c - Lower_c
```
...
**Answer 12:** Based off teh graphs you can see that the days the ice off date will be is going down more in the post 1970 era rather than before. You see mutiple days go below 100 in the post 1970 graph which has much less data within side it. While the pre 1970's data has alot more data and around the same amount of days below 100 on the calender. 
So yes the ice off day is changing mroe quickly and you can tell by the slop of the lines and the line of best fit. The grap does look like the pre 1970 is stepper bu that is becuase of the longer period of which data was capture so for the time the post 1970s ice off days is changing quicker.
...
```{r}
library(patchwork)
pre <- (sunapee |> filter(year < 1970))
post <- (sunapee |> filter(year > 1970))
G1<-ggplot(pre, aes(x=year,y=ice_off_doy))+
  geom_line()+
  geom_point()+
  geom_abline(
    intercept = coef(fit)[1],
    slope = coef(fit)[2],
    color = "red")
G2 <-ggplot(post,aes(x=year,y=ice_off_doy))+
  geom_line()+
  geom_point()+
  geom_abline(
    intercept = coef(fit)[1],
